...
def get_post_prompt_v2(query, num ,entities = ""):
    if entities == "":
        return f"Search Query: {query}. \nRank the {num} passages above based on their relevance and usefulness to anwser the query {query}.The passages should be listed in descending order of relevance using identifiers. The most relevant passages should be listed first. The output format should be **RANKING:** [] > [] > etc.. Only response the ranking result, do not say any word or explain."
    else:
        return f"Search Query: {query}. \nRank the {num} passages above based on their relevance and usefulness to anwser the query {query}. Higher relevance should be assigned if a passage directly mention the next entities: {entities}. The passages should be listed in descending order using identifiers. The most relevant passages should be listed first. The output format should be **RANKING:** [] > [], e.g., **RANKING:** [1] > [2]. Only response the ranking results, do not say any word or explain."
...
def create_permutation_instruction_v2(query, docs ,entities=''):

    num = len(docs)
    if num > 20:
        raise ValueError("Reduce the number of documets to 20..")
    max_length = 300
    ...
    messages.append({'role': 'user', 'content': get_post_prompt(query, num, entities)})
...
def receive_permutation_v2(item, permutation, rank_start=0, rank_end=100):
...
def permutation_pipeline(item=None, rank_start=0, rank_end=100, model_name='gpt-4o-mini', api_key=None):
    ...
    backend_type = "openai" if "gpt" in model_name else "ollama"

    handler = ModelHandler(model_name=model_name, backend_type=backend_type)
    permutation = handler.generate_answer(
        messages,
        temperature=0,
        debug = False
    )

...
def display_answer(text):
    colAnswer.subheader("🛠️ Answer")
    colAnswer.markdown(
        f"""
<div style="border: 2px solid #ccc; background-color: rgb(240,242,246); 
    padding: 0px; max-height: 70vh; min-height: 200px; height: auto; 
    overflow-y: auto; color: black; scroll-behavior: smooth;">
    {text['message']}
</div>
...
def visualize_ner(
    ...
    *,
    ...
    if not labels:
        ...
        if marco_option:
            colAnswer.write(f"{style}{get_html(html)}", unsafe_allow_html=True)
        else:
            container.write(f"{style}{get_html(html)}", unsafe_allow_html=True)


def display_docs(result, ner_extraction, environment, expander_option):
    print("onix si si ")
    scores_docs = ["" for x in range(10)]
    for i, line in enumerate(result):
        if i == 10:
            break

        if "error" in line.keys():
            container.warning(line["error"], icon="⚠️")
            container.divider()
            break
        else:
            # print("*********************************************")
            ...
            if st.session_state.optionSorting == "keyword matching":
                ...
                if environment == "production":
                    container.markdown(f"<strong style='font-size: 20px;'>{line['title']}</strong>", unsafe_allow_html=True)
                if environment == "test":
                    container.markdown(
            ...
            col1, col2, col3, col4 = container.columns([0.3,0.2,0.15,0.25])
            with col1:
                st.caption(f"Link: http://doi.org/{line['doi']}") 
            with col2:
                st.caption(f"Section: {line.get('titleSection', '') }") 
            with col3:
...
colLogo, colInput, colSearchBtn, colSorting, colNERSel, colOpt = st.columns( [0.15, 0.3, 0.09, 0.12, 0.102 , 0.2] , vertical_alignment="bottom")

#colLogo.markdown(new_title, unsafe_allow_html=True)
colLogo.image("SearchEngineLogo2.jpg")
input_query = colInput.text_input("", value="", autocomplete="on")
#input_query = colInput.chat_input("Type Query" , on_submit=change_name , args=[input_query, optionModel])

colNERSel.caption("Choose NER model")
with colNERSel:
...
buttonSearch = colSearchBtn.button(
    label="",
    key="a2a_buttonRet",
...
option = colSorting.toggle(toggle_label, value=toggle_value, key="my_toggle")



colOpt.caption("Active AI Answer")
on = colOpt.toggle("Feature", value=True, label_visibility="visible")

st.write("In this search engine you can use full questions and keyword to get results")

###################   USER OPTIONS ############################


#colDocuments, colAnswer = st.columns( [0.65, 0.35] , vertical_alignment="top" , gap="medium")
colAnswer, colDocuments = st.columns( [0.35, 0.65] , vertical_alignment="top" , gap="medium")
...
if st.session_state.ON_STATE == True and input_query != "":
    ...
    colAnswer.header("📑 Query", divider="blue")
    colDocuments.header("📑 Documents related", divider="blue")
...
if buttonSearch:
    ...
    if st.session_state.optionSorting == False:
        print("Sparse........")
...
if st.session_state.AnswerGenAI:
    ...
    colAnswer.markdown("<br>", unsafe_allow_html=True)  # Add space
...
if st.session_state.ranked_docs:
    ...
    #     icon="ℹ️",
    # )
    container = colDocuments.container(height= 890 ,  border=False)
